...
app.get("/getBoilerTypeById/:id", (req, res) => {
  const found = boilerTypes.some((boilerTypes) => {
    boilerTypes.id === parseInt(req.params.id);
  });
...
app.get("/getBoilerTypesByDescription/:description", (req, res) => {
  ...
  if (found) {
    ...
    res.status(400).json({
      msg: `No boiler type with the description of ${req.params.description}`,
    });
  }
});
...
app.get("/getBoilerTypesByStock/:stock", (req, res) => {
  const found = boilerTypes.some((boilerTypes) => {
    boilerTypes.stock === parseInt(req.params.stock);
  });
  ...
  if (found) {
    ...
    res
      .status(400)
      .json({ msg: `No boiler type with the stock of ${req.params.stock}` });
  }
});

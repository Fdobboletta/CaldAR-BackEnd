...
app.get("/getTechnicianById/:id", (req, res) => {
  const found = technicians.filter(
    (technician) => technician.id === parseInt(req.params.id)
  );
...
app.get("/getTechnicianByFirstName/:first_name", (req, res) => {
  const found = technicians.filter(
    (technician) => technician.first_name === String(req.params.first_name)
  );
  ...
  if (found.length > 0) {
    ...
    res
      .status(400)
      .json({ msg: `No technician with the name of ${req.params.first_name}` });
  }
});
//getTechnicianByLastName
app.get("/getTechnicianByLastName/:last_name", (req, res) => {
  const found = technicians.filter(
    (technician) => technician.last_name === String(req.params.last_name)
  );
  ...
  if (found.length > 0) {
    ...
    res.status(400).json({
      msg: `No technician with the last name of ${req.params.last_name}`,
    });
  }
});
...
app.get("/gettechnicianByEmail/:email", (req, res) => {
  const found = technicians.filter(
    (technician) => technician.email === String(req.params.email)
  );
  ...
  if (found.length > 0) {
    ...
    res
      .status(400)
      .json({ msg: `No technician with the email of ${req.params.email}` });
  }
});
...
app.get("/getTechnicianBySkillsId/:skillsId", (req, res) => {
  ...
  if (found.length > 0) {
    ...
    res.status(400).json({
      msg: `No technician with the skill ID of ${req.params.skillsId}`,
    });
  }
});
...
app.get("/getTechnicianByHourRate/:hour_rate", (req, res) => {
  ...
  if (found.length > 0) {
    ...
    res.status(400).json({
      msg: `No technician with the hour rate of ${req.params.hour_rate}`,
    });
  }
});
...
app.get("/getTechnicianByDailyCapacity/:dailyCapacity", (req, res) => {
  ...
  if (found.length > 0) {
    ...
    res.status(400).json({
      msg: `No technician with the daily capacity of ${req.params.daily_capacity}`,
    });
  }
});

...
app.get("/getBuildingById/:id", (req, res) => {
  const found = buildings.filter(
    (building) => building.id === parseInt(req.params.id)
  );
...
app.get("/getBuildingByAddress/:address", (req, res) => {
  const found = buildings.filter(
    (building) => building.address === String(req.params.address)
  );
  ...
  if (found.length > 0) {
    ...
    res
      .status(400)
      .json({ msg: `No building with the address of ${req.params.address}` });
  }
});
...
app.get("/getBuildingByBoilersId/:boilersId", (req, res) => {
  ...
  const found = buildings.filter((building) =>
    building.boilersId.includes(boilersIdNumber)
  );
  ...
  if (found.length > 0) {
    ...
    res.status(400).json({
      msg: `No building with the boiler ID of ${req.params.boilersId}`,
    });
  }
});
...
app.get("/getBuildingsByFullName/:fullName", (req, res) => {
  const found = buildings.filter(
    (building) => building.fullName === String(req.params.fullName)
  );
  ...
  if (found.length > 0) {
    ...
    res
      .status(400)
      .json({ msg: `No building with the name of ${req.params.fullName}` });
  }
});
//getBuildingByPhone
app.get("/getBuildingsByPhone/:phone", (req, res) => {
  const found = buildings.filter(
    (building) => building.phone === String(req.params.phone)
  );
  ...
  if (found.length > 0) {
    ...
    res
      .status(400)
      .json({ msg: `No building with the phone of ${req.params.phone}` });
  }
});

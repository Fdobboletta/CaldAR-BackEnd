...
app.get("/getBoilerById/:id", (req,res) => {
    ...
    if (found) {
        res.status(200).json(boilers.filter(boiler => boiler.id === parseInt(req.params.id)));
    } else {
...
app.get("/getBoilerByTypeId/:typeId", (req,res) => {
    ...
    if (found) {
        res.status(200).json(boilers.filter(boiler => boiler.typeId === parseInt(req.params.typeId)));
    } else {
...
app.get("/getBoilerByMaintainceRate/:maintaince_rate", (req,res) => {
    ...
    if (found) {
        res.status(200).json(boilers.filter(boiler => boiler.maintaince_rate === String(req.params.maintaince_rate)));
    } else {
...
app.get("/getBoilerByHourMaintainceCost/:hour_maintaince_cost", (req,res) => {
    ...
    if (found) {
        res.status(200).json(boilers.filter(boiler => boiler.hour_maintaince_cost === String(req.params.hour_maintaince_cost)));
    } else {
...
app.get("/getBoilerByHourEventualCost/:hour_eventual_cost", (req,res) => {
    ...
    if (found) {
        res.status(200).json(boilers.filter(boiler => boiler.hour_eventual_cost === String(req.params.hour_eventual_cost)));
    } else {

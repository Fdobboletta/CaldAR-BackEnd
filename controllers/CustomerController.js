...
app.get("/getCustomerByCustomerType/:customerType", (req, res) => {
  const found = customers.some(
    (customer) => customer.customerType === String(req.params.customerType)
  );
  if (found) {
    res
      .status(200)
    ...
    res.status(400).json({
      msg: `No customer with the customer type ${req.params.customerType}`,
    });
  }
});
...
app.get("/getCustomerByEmail/:email", (req, res) => {
  const found = customers.some(
    (customer) => customer.email === String(req.params.email)
  );
  if (found) {
    res
      .status(200)
    ...
    res
      .status(400)
      .json({ msg: `No customer with the customer type ${req.params.email}` });
  }
});
...
app.get("/getCustomerByBuilding/:buildings", (req, res) => {
  ...
  const found = customers.filter((customer) =>
    customer.buildings.includes(buildingInt)
  );
  if (found.length > 0) {
    res.status(200).json(found);
  } else {
    res
      .status(400)
      .json({ msg: `No customer with the customer type ${buildingInt}` });
  }
});
...
app.get("/getCustomerByFiscalAddress/:fiscal_address", (req, res) => {
  const found = customers.some(
    (customer) => customer.fiscal_address === String(req.params.fiscal_address)
  );
  if (found) {
    res
      .status(200)
    ...
    res.status(400).json({
      msg: `No customer with the customer type ${req.params.fiscal_address}`,
    });
  }
});
